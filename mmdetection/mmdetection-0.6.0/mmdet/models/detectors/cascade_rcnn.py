...
# 根据如下的语句，将类名——CascadeRCNN作为形参传入了register_module
...
class CascadeRCNN(BaseDetector, RPNTestMixin):
    ...
    def __init__(self,
        ...
        # 创建backbone组件模型——类型是ResNet，已经在register中注册过了，具体位置在models/backbones/resnet.py中
        self.backbone = builder.build_backbone(backbone)

        # 创建neck组件模型——类型是FPN，已经在registers中注册过了，具体位置是necks/fpn.py
        # 如果找不到就会报错说：未实现错误
        if neck is not None:
        ...
        # 创建build_head组件模型——类型是RPNHead，已经在register中注册了，具体位置在anchor_heads/rpn_head
        if rpn_head is not None:
            self.rpn_head = builder.build_head(rpn_head)

        # 创建bbox_roi_extractor组件——类型是SingleRoIExtractor，已经注册了，具体位置是roi_extractors/single_level
        # 创建bbox_heads组件——类型是SharedFCBBoxHead，已经注册了，具体位置是bbox_heads/convfc_bbox_head
        if bbox_head is not None:
            # bbox_roi_extractor= SingleRoIExtractor
            self.bbox_roi_extractor = nn.ModuleList()
            self.bbox_head = nn.ModuleList()
            # 如果bbox_roi_extractor不是一个list，那就意味着就一个网络，所以复制num_stages遍构成一个list
            if not isinstance(bbox_roi_extractor, list):
                bbox_roi_extractor = [
                    bbox_roi_extractor for _ in range(num_stages)
                ]
            # bbox_head是一个list，所以跳过，否则就复制num_stages次构建成list类型，这边是3
            if not isinstance(bbox_head, list):
                bbox_head = [bbox_head for _ in range(num_stages)]
            assert len(bbox_roi_extractor) == len(bbox_head) == self.num_stages
            for roi_extractor, head in zip(bbox_roi_extractor, bbox_head):
                # 创建3级的 bbox_head 和 bbox_roi_extractor，都是nn.ModuleList类型
